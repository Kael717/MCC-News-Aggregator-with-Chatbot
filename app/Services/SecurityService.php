...
class SecurityService
{
     ...
     */
    public function validateInput($input, $fieldName = 'input')
    {
        if (!is_string($input)) {
            return ['valid' => true];
        }

        $dangerousPatterns = config('security.input_validation.dangerous_patterns', []);
    ...
    private function isSuspiciousInput($input)
    {
        ...
        if ($specialCharCount > strlen($input) * 0.3) {
            return true;
        }
